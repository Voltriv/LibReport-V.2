        ...
        {(title || description || headerHasActions) && (
          <header className="mb-8 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            <div className="flex-1">
              ...
              {title ? (
                <h1 className="text-3xl font-bold text-slate-900 dark:text-stone-100">{title}</h1>
              ) : null}
              {description ? (
                <p className="text-slate-600 dark:text-stone-400 mt-1">{description}</p>
              ) : null}
              {meta ? <div className="mt-2 text-sm text-slate-500 dark:text-stone-400">{meta}</div> : null}
            </div>
            {headerHasActions ? (
              <div className={`flex flex-wrap items-center gap-3 ${actionsClassName}`}>
                ...
                {!hideUserMenu && (
                  <div className="relative" ref={menuRef}>
                    ...
                    {isMenuOpen && (
                      <div className="absolute right-0 mt-3 w-52 rounded-xl theme-panel ring-1 ring-slate-200 dark:ring-stone-700 shadow-xl p-2 z-50">
                        <button
                          type="button"
