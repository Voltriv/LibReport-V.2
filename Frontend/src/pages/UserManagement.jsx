...
const UserManagement = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const items = (r.data || []).map(u => ({ id: u._id || u.id, fullName: u.fullName || u.name || '-', studentId: u.studentId || '-', course: u.role || '-', registrationDate: u.createdAt ? new Date(u.createdAt).toLocaleString() : '-', status: 'Active' }));
      setUsers(items);
    } catch (e) { setIsModalOpen(false); }
  };

  const handleInputChange = (e) => { const { name, value } = e.target; setSelectedUser((prev) => ({ ...prev, [name]: value })); };

  const handleLogout = () => { setShowLogoutModal(false); setIsDropdownOpen(false); try { localStorage.removeItem('lr_token'); localStorage.removeItem('lr_user'); } catch {} navigate("/signin", { replace: true }); };
      ...
      <main className="px-4 md:pl-6 lg:pl-8 pr-4 py-6 md:ml-72">
        <div className="flex items-center justify-end">
          <div className="relative">
            <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="inline-flex items-center gap-2 rounded-full bg-white/80 dark:bg-stone-900/60 ring-1 ring-slate-200 dark:ring-stone-700 px-2 py-1 shadow hover:shadow-md">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                {users.map((user) => (
                  <tr key={user.id} className="text-slate-800 dark:text-stone-100">
                    ...
                    <td className="py-2 pr-4"><span className="inline-flex items-center rounded px-2 py-0.5 bg-emerald-600 text-white text-xs">{user.status}</span></td>
                    <td className="py-2 pr-4 text-right">
                      <button className="rounded-lg px-3 py-1.5 ring-1 ring-slate-200 dark:ring-stone-700 bg-white dark:bg-stone-950 text-slate-700 dark:text-stone-200" onClick={() => handleEdit(user)}>Edit</button>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div className="w-full max-w-md rounded-xl bg-white dark:bg-stone-900 ring-1 ring-slate-200 dark:ring-stone-700 p-6">
              ...
              <div className="mt-4 space-y-3">
                <div>
                  ...
                  <input type="text" name="fullName" value={selectedUser.fullName} onChange={handleInputChange}
                ...
                <div>
                  ...
                  <input type="text" name="studentId" value={selectedUser.studentId} onChange={handleInputChange}
                         className="mt-1 w-full rounded-lg border border-slate-300 dark:border-stone-600 bg-white dark:bg-stone-950 px-3 py-2 text-slate-900 dark:text-stone-100" />
                </div>
                <div>
                  <label className="block text-sm text-slate-600 dark:text-stone-300">Role</label>
                  <input type="text" name="course" value={selectedUser.course} onChange={handleInputChange}
                         className="mt-1 w-full rounded-lg border border-slate-300 dark:border-stone-600 bg-white dark:bg-stone-950 px-3 py-2 text-slate-900 dark:text-stone-100" />
                </div>
                <div>
                  <label className="block text-sm text-slate-600 dark:text-stone-300">Status</label>
                  <input type="text" name="status" value={selectedUser.status} onChange={handleInputChange}
                         className="mt-1 w-full rounded-lg border border-slate-300 dark:border-stone-600 bg-white dark:bg-stone-950 px-3 py-2 text-slate-900 dark:text-stone-100" />
                </div>
              </div>
              <div className="mt-6 flex items-center justify-end gap-3">
                <button className="rounded-lg px-4 py-2 ring-1 ring-slate-200 dark:ring-stone-700 bg-white dark:bg-stone-950 text-slate-700 dark:text-stone-200" onClick={() => setIsModalOpen(false)}>Cancel</button>
                <button className="rounded-lg px-4 py-2 bg-brand-gold text-white hover:opacity-90" onClick={handleSave}>Save</button>
              </div>
            </div>
          </div>
        )}
